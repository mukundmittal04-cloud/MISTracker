      ...
      try{
        ...
        if(_qId && _reaps.items[_qId] && !_reaps.items[_qId].resolved){
          ...
          if(_v2){
            ...
            try{ await waClient.sendMessage(CONFIG.APPROVAL_GROUP_JID, '\u2713 recorded '+_whoR+"'s "+(_v2==='yes'?'re-approval':_v2)+' \u2014 '+_rp.label); }catch(e){}
            if(_v2==='no'){
        ...
        if(_qId && /^\s*\*?EXPENSE REQUEST/i.test(_qBody)){
          ...
          if(_v){
            ...
            try{ await waClient.sendMessage(CONFIG.APPROVAL_GROUP_JID, '\u2713 recorded '+_whoL+"'s "+(_v==='yes'?'approval':_v)+' \u2014 '+_lbl+(_amt?' Rs.'+formatINR(_amt):'')); }catch(e){}
