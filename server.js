// ============================================================
// FIDATO MIS SERVER v2.8.1 — cleaner categorised WhatsApp digest format
...
async function buildApprovalReminderDigest() {
  ...
  function section(title, note, arr){
    ...
    arr.forEach(function(e){
      ...
      lines.push('Rs.'+formatINR(e.approvedAmount||e.amount)+' · '+age(e)+q);
      items.push({n:n, id:e.id, label:label(e), amount:e.approvedAmount||e.amount, sender:e.sender});
    });
  }
  section('Needs M', 'S already approved', onM);
  section('Needs S', 'M already approved', onS);
  section('Needs both M & S', '', onBoth);
  if(held.length){
    lines.push('');
    ...
    held.forEach(function(e){
      ...
      n++;
      lines.push('*'+n+'.* '+label(e));
      lines.push('Rs.'+formatINR(e.amount)+' · held by '+who+' '+age(e));
      items.push({n:n, id:e.id, label:label(e), amount:e.approvedAmount||e.amount, sender:e.sender});
    });
  }
  lines.push('');
...
app.get('/api/report-status',function(req,res){res.json({botEnabled:CONFIG.BOT_ENABLED,whatsapp:waReady,version:'2.8.1',visionEnabled:CONFIG.CLAUDE_API_KEY?true:false,reverseScanWindowDays:REVERSE_SCAN_WINDOW_DAYS,reverseScanMinAmount:REVERSE_SCAN_MIN_AMOUNT});});
...
app.listen(CONFIG.PORT,function(){
  console.log('\nFidato MIS Server v2.8.1 | Port:',CONFIG.PORT,'| Vision:',CONFIG.CLAUDE_API_KEY?'enabled':'disabled');
